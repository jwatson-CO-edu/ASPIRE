########## DEV PLAN ################################################################################
"""
[>] One sample per solve!
    * This is a test to see if the "live" approach causes insoluble inconsistencies
    [Y] Write cached sampler, 2024-02-27: One-Solve:One-Sample!
    [>] Test using best settings from last round
"""
...
from env_config import ( _GRASP_VERT_OFFSET, _GRASP_ORNT_XYZW, _NULL_NAME, _ACTUAL_NAMES, _MIN_X_OFFSET,
                         _ONE_SAMPLE_SOLN, _BLOCK_SCALE, _CLOSEST_TO_BASE, _ACCEPT_POSN_ERR, _MIN_SEP, 
                         _Z_SAFE, _N_POSE_UPDT, _WP_NAME, _SAMPLE_DET )
...
class ResponsiveExecutive:
    ...
    def sample_cached( self, objName ):
        """ Get the object from the One-Solve:One-Scan strat """
        for sym in self.symbols:
            if sym.label == objName:
                return sym
        return None
    ...
    def get_object_stream( self ):
        ...
        def stream_func( *args ):
            ...
            if _SAMPLE_DET:
                ...
                if _ONE_SAMPLE_SOLN:
                    rtnObj = self.sample_cached( objcName )
                else:
    ...
    def get_init_predicates( self ):
        ...
        ## Sample Symbols ##
        if _SAMPLE_DET:
            rtnObj = self.world.full_scan_true()
        else:
            # rtnObj = self.memory.sample_consistent( objcName )
            rtnObj = self.memory.scan_consistent_fresh()
        for obj in rtnObj:
            self.add_grasp_config_to_object( obj )

        for obj in rtnObj:
            self.facts.extend( [ ('GraspObj', obj.label, obj,), ('Waypoint', obj,), ] )
    ...
    def phase_3_Plan_Task( self ):
        ...
        try:
            ...
            solution = solve( 
                ...
                reorder = True, # Setting to false bare impacts sol'n time
                # planner = planner
    ...
    def phase_4_Execute_Action( self ):
        ...
        btr = BT_Runner( self.action, self.world, 30.0 )
