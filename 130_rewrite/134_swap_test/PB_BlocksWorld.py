...
from env_config import ( TABLE_URDF_PATH, _MIN_X_OFFSET, _BLOCK_SCALE, _CONFUSE_PROB, _BLOCK_NAMES,
                         ...
                         _ACTUAL_NAMES, _ROBOT_SPEED, _ONLY_RED_GREEN, )
...
class PB_BlocksWorld:
    ...
    def setup_two_towers_problem( self ):
    ...
    def setup_swap_problem( self ):
        """ The problem being studied """

        redBlock = make_block( self.physicsClient )
        self.physicsClient.changeVisualShape( redBlock, -1, rgbaColor=[1.0, 0.0, 0.0, _BLOCK_ALPHA] )
        ornt = _trgtGrn.pose.tolist()[4:] + [_trgtGrn.pose.tolist()[3],]
        self.physicsClient.resetBasePositionAndOrientation( redBlock, _trgtGrn.pose.tolist()[:3], ornt )

        grnBlock = make_block( self.physicsClient )
        self.physicsClient.changeVisualShape( grnBlock, -1, rgbaColor=[0.0, 1.0, 0.0, _BLOCK_ALPHA] )
        ornt = _trgtRed.pose.tolist()[4:] + [_trgtRed.pose.tolist()[3],]
        self.physicsClient.resetBasePositionAndOrientation( grnBlock, _trgtRed.pose.tolist()[:3], ornt )

        self.blocks = [redBlock, grnBlock, None]
    ...
    def __init__( self, graphicsOverride = False ):
        ...
        if _USE_GRAPHICS or graphicsOverride:
            self.physicsClient = bc.BulletClient( connection_mode = pb.GUI ) # or pb.DIRECT for non-graphical version
        else:
        ...
        ## Instantiate Blocks ##
        if _ONLY_RED_GREEN:
            self.setup_swap_problem()
        else:
            self.setup_two_towers_problem()
    ...
    def reset_blocks( self ):
        ...
        if _ONLY_RED_GREEN:
            ...
            ornt = _trgtGrn.pose.tolist()[4:] + [_trgtGrn.pose.tolist()[3],]
            self.physicsClient.resetBasePositionAndOrientation( self.get_handle( 'redBlock' ), _trgtGrn.pose.tolist()[:3], ornt )

            ornt = _trgtRed.pose.tolist()[4:] + [_trgtRed.pose.tolist()[3],]
            self.physicsClient.resetBasePositionAndOrientation( self.get_handle( 'grnBlock' ), _trgtRed.pose.tolist()[:3], ornt )
